...
x_train, x_test, y_train, y_test = train_test_split(data['scaledTumorImage'], data['label'], 
                                                    test_size=0.10, random_state=10)

x_train, x_test, y_train, y_test = train_test_split(scaledTumorImages, labels, 
                                                    test_size=0.10, random_state=10)
...
for x in range(w):
    for y in range(h):
        check[x][y] = int(scaledTumorImages[0][x][y])
        
check = scaledTumorImages[0]

for x in scaledTumorImages:
    print(x[0])

a = np.array([[int(j) for j in i.split('\t')] for i in check.splitlines()])

import csv

with open('Output.txt', 'w') as f:
    csvwriter = csv.writer(f)
    csvwriter.writerows(scaledTumorImages)

dff = pd.read_csv('Output.txt', header=None, delimiter=',')

checker = dff[0][0]

# Get Brain Tumor data
x_train, x_test, y_train, y_test = train_test_split(scaledTumorImages, labels, 
                                                    test_size=0.10, random_state=10)
...
img_rows, img_cols= x_train[0].shape[0:3]
